...
program SimulationfullLangevintwopointo
  ...
  !-----Simulation Control Parameters---- check
  ...
  open(unit=12, file="velocity_distributions50.txt", status='replace', action='write')
  ...
  open(unit=13, file="position_distributions50.txt", status='replace', action='write')
  ...
  open(unit=14, file="joint_distribution50.txt", status='replace', action='write')
  ...
  open(unit=15, file="phase_space_points50.txt", status='replace', action='write')
  ...
  print*, "Done. Files:"
  print*, "  velocity_distributions50.txt"
  print*, "  position_distributions50.txt"
  print*, "  joint_distribution50.txt, joint_theory50.txt"
  print*, "  axes_theta_theory50.txt, axes_theta_sim50.txt"
  print*, "  diagnostics_Tmax50.txt"
...
!  - joint_theory50.txt: theory P(x,v) on same grid as hist
!  - diagnostics_Tmax50.txt: theory vs sample stats
!  - axes_theta_theory50.txt, axes_theta_sim50.txt: two perpendicular lines each
!========================================================
subroutine write_theory_and_diagnostics(tnow, m, gamma, kBT, &
  ...
  ! locals
  integer :: i, ix, iv, u_the, u_inf, u_ax_th
  real*8 :: PI, tau, a, D
  ...
  ! --- sample stats from particles (for diagnostics only) ---
  ...
  ! --- diagnostics file (kept) ---
  finfo = "diagnostics_Tmax50.txt"
  ...
  !  ! --- THEORY axes (two perpendicular lines through origin, 2 columns: x v) ---
  Lx = dsqrt(sigx2_th)
  Lv = dsqrt(sigv2_th)
  L  = max(Lx, Lv)     ! ~±1σ length each way; tweak if too long/short

  cth = dcos(theta_th);  sth = dsin(theta_th)
  open(newunit=u_ax_th, file="axes_theta_theory50.txt", status='replace', action='write')
  write(u_ax_th,*) "# two perpendicular lines at theta_th (x v)"
  ! along theta
  write(u_ax_th,'(2E20.8)') -L*cth, -L*sth
  write(u_ax_th,'(2E20.8)')  L*cth,  L*sth
  write(u_ax_th,*)
  ! perpendicular
  write(u_ax_th,'(2E20.8)') -L*sth,  L*cth
  write(u_ax_th,'(2E20.8)')  L*sth, -L*cth
  close(u_ax_th)
