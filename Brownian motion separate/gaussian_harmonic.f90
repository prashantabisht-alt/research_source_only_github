program Brownian_motion_gaussian_harmonic
  implicit none
  ...
  real*8 :: dt, diff, eta, position, gaussianvariable, kBT, gamma, k
  ...
  xmin = -10.0d0     ! Histogram range (confined by potential)
  xmax = 10.0d0
  ...
  ! Simulation loop: Gaussian noise with harmonic potential
  do j = 1, ntraj
    if (mod(j, 100000) == 0) print *, "Gaussian Trajectory:", j
    position = 0.0d0
    do timestep = 1, Tmax
      call gaussian(gaussianvariable)
      eta = gaussianvariable * dsqrt(2.0d0 * diff / dt)  ! Gaussian noise scaled
  ...
  ! Write MSD to file
  open(unit=44, file="msd_gauss_harmonic.txt")
  do i = 1, Tmax
    write(44, *) i * dt, msd(i)
  end do
  close(44)
  ...
  call write_pdf(position_hist, nbins, xmin, xmax, ntraj, "pdf_gauss_harmonic.txt")

  ! Deallocate arrays
  deallocate(position_hist, msd)

  print *, "Gaussian simulation complete. Outputs: pdf_gauss_harmonic.txt, msd_gauss_harmonic.txt"

end program Brownian_motion_gaussian_harmonic
