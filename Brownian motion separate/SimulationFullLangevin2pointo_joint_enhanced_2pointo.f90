...
!  - P(x,v,t): bivariate Gaussian; write theory grid
!  - Axes: write **major/minor** (eigenvector) axes to *20point2* files
!==========================================================
program SimulationfullLangevintwopointo
  ...
  open(unit=72, file="velocity_distributions20.txt", status='replace', action='write')
  ...
  open(unit=90, file="position_distributions20.txt", status='replace', action='write')
  write(90,*) "# x_center, P_simulated, P_theoretical (exact cold-start variance)"
  ...
  open(unit=110, file="joint_distribution20.txt", status='replace', action='write')
  write(110,*) "# x_center, v_center, P_joint(x,v) @ T_max"
  do ix = 1, num_bins
  ...
  ! Raw phase-space points (for scatter/ellipse overlays)
  open(unit=150, file="phase_space_points.txt", status='replace', action='write')
  ...
  !==== THEORY GRID + DIAGNOSTICS + EIGEN-AXES (major/minor) ====
  ...
  print*, "Done. Files:"
  print*, "  velocity_distributions20.txt"
  print*, "  position_distributions20.txt"
  print*, "  joint_distribution20.txt, joint_theory20.txt"
  print*, "  axes_major_minor_theory20point2.txt, axes_major_minor_sim20point2.txt"
  print*, "  diagnostics_Tmax20.txt"
...
!========================================================
! Theory overlay + diagnostics + **eigen** principal axes (major/minor)
!  - joint_theory20.txt: theory P(x,v) on same grid as hist
!  - diagnostics_Tmax20.txt: theory vs sample stats
!  - axes_major_minor_theory20point2.txt, axes_major_minor_sim20point2.txt
!========================================================
subroutine write_theory_and_diagnostics(tnow, m, gamma, kBT, &
  ...
  real*8 :: Lplot
  ! eigen stuff
  real*8 :: lamP_th, lamM_th, uxP_th, uvP_th, uxM_th, uvM_th
  real*8 :: lamP_si, lamM_si, uxP_si, uvP_si, uxM_si, uvM_si
  ...
  ! ---- eigenvector principal axes (major/minor), new *20point2 files ----
  Lplot = 0.5d0 * dsqrt( (x_max - x_min)**2 + (v_max - v_min)**2 )

  ! THEORY eigen-axes
  call eig2x2(sigx2_th, cov_th, sigv2_th, lamP_th, lamM_th, uxP_th, uvP_th, uxM_th, uvM_th)
  open(newunit=u_ax_th, file="axes_major_minor_theory20point2.txt", status='replace', action='write')
  write(u_ax_th,*) "# MAJOR axis (theory): two endpoints (z=0 for splot)"
  write(u_ax_th,'(3E20.8)') -Lplot*uxP_th, -Lplot*uvP_th, 0.0d0
  write(u_ax_th,'(3E20.8)')  Lplot*uxP_th,  Lplot*uvP_th, 0.0d0
  write(u_ax_th,*)
  write(u_ax_th,*) "# MINOR axis (theory): two endpoints"
  write(u_ax_th,'(3E20.8)') -Lplot*uxM_th, -Lplot*uvM_th, 0.0d0
  write(u_ax_th,'(3E20.8)')  Lplot*uxM_th,  Lplot*uvM_th, 0.0d0
  close(u_ax_th)

  ! SIM eigen-axes
  call eig2x2(xvar, cov, vvar, lamP_si, lamM_si, uxP_si, uvP_si, uxM_si, uvM_si)
  open(newunit=u_ax_si, file="axes_major_minor_sim20point2.txt", status='replace', action='write')
  write(u_ax_si,*) "# MAJOR axis (sim): two endpoints (z=0 for splot)"
  write(u_ax_si,'(3E20.8)') -Lplot*uxP_si, -Lplot*uvP_si, 0.0d0
  write(u_ax_si,'(3E20.8)')  Lplot*uxP_si,  Lplot*uvP_si, 0.0d0
  write(u_ax_si,*)
  write(u_ax_si,*) "# MINOR axis (sim): two endpoints"
  write(u_ax_si,'(3E20.8)') -Lplot*uxM_si, -Lplot*uvM_si, 0.0d0
  write(u_ax_si,'(3E20.8)')  Lplot*uxM_si,  Lplot*uvM_si, 0.0d0
  close(u_ax_si)

  print *, "Wrote joint_theory20.txt, diagnostics_Tmax20.txt,"
  print *, "      axes_major_minor_theory20point2.txt, axes_major_minor_sim20point2.txt"

contains
  pure subroutine eig2x2(a11,a12,a22, lam_plus, lam_minus, ux_plus, uv_plus, ux_minus, uv_minus)
    ! Symmetric 2x2 eigenpairs for A = [[a11,a12],[a12,a22]]
    real*8, intent(in)  :: a11, a12, a22
    real*8, intent(out) :: lam_plus, lam_minus
    real*8, intent(out) :: ux_plus, uv_plus, ux_minus, uv_minus
    real*8 :: tr, disc, nx, ny, nrm, lp, lm

    tr   = a11 + a22
    disc = tr*tr - 4.0d0*(a11*a22 - a12*a12)
    if (disc < 0d0) disc = 0d0
    lp = 0.5d0*(tr + dsqrt(disc))
    lm = 0.5d0*(tr - dsqrt(disc))
    lam_plus  = lp
    lam_minus = lm

    ! Eigenvector for lambda+ solves (A - lp I) u = 0
        ! Eigenvector for lambda+ solves (A - lp I) u = 0
    if (abs(a12) > 1.0d-300) then
       nx = a12
       ny = lp - a11
    else
       ! nearly diagonal: pick axis with larger variance
       if (a11 >= a22) then
          nx = 1.0d0; ny = 0.0d0
       else
          nx = 0.0d0; ny = 1.0d0
    ...
    nrm = dsqrt(nx*nx + ny*ny); if (nrm < 1d-300) nrm = 1d0
    ux_plus = nx/nrm; uv_plus = ny/nrm

    ! orthogonal eigenvector (2D): rotate by +90°
    ux_minus = -uv_plus
    uv_minus =  ux_plus
