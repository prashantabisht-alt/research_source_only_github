...
!=====================================================================
program pure_jerky_persistent_fig3a
  implicit none
  ...
  real(dp), parameter :: taup      = 0.001_dp   ! OU persistence time τp
  real(dp), parameter :: v0        = 1.0_dp   ! OU stationary std

  real(dp), parameter :: dt   = 0.0005_dp      ! time step (units of τp=1 here)
  real(dp), parameter :: Tmax = 15.0_dp       ! total time ( 20 τp covers both regimes)
  integer,  parameter :: Nsteps = nint(Tmax/dt)

  integer,  parameter :: Ntraj  = 100     ! ensemble size (should raise for smoother curves)

  
  real(dp), allocatable :: msd(:), tgrid(:) !accumulated msd iver trajectories and time grid
  real(dp) :: normY
  integer  :: it, n, uout
  integer  :: seed
...
contains
  ...
  subroutine run_persistent_jerky(msd, Ntraj, Nsteps, dt, lambda, gamma_act, taup, v0)
    ...
    !integer :: u1, u2, u3
    !open(newunit=u1, file="traj1.txt", status="replace")
    !open(newunit=u2, file="traj2.txt", status="replace")
    !open(newunit=u3, file="traj3.txt", status="replace")
    ...
    do n = 1, Ntraj
      ...
      do it = 1, Nsteps
        ...
        !if (n == 1) write(u1,'(2(1X,ES20.12))') it*dt/taup, x
        !if (n == 2) write(u2,'(2(1X,ES20.12))') it*dt/taup, x
        !if (n == 3) write(u3,'(2(1X,ES20.12))') it*dt/taup, x
...
end program pure_jerky_persistent_fig3a
include 'mt.f90'
