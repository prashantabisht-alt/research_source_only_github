program alpha_from_msd
  ...
  character(len=*), parameter :: infile  = "msd_persistent_norm.txt"
  character(len=*), parameter :: outfile = "alpha_persistent.txt"
  integer,  parameter :: SMOOTH_WIN = 0        ! 0 = off, 2 ~ mild smoothing
  real(dp), parameter :: eps = 1.0e-14_dp      ! guard for duplicate s
  ...
  ! ---------- count numeric rows (ignore # and blanks) ----------
  nmax = 0
  ...
  close(uin)
  if (nmax < 5) then
    print *, "ERROR: need >= 5 data rows (got ", nmax, ")"; stop 1
  end if
  ...
  ! --------- d ln(MSD) / d ln s (central where possible) ---------
  do i = 1, n
    if (.not. good(i)) then
      a(i) = 0.0_dp
  ...
  ! --------- optional smoothing (moving average ±win) ---------
  win = SMOOTH_WIN
  if (win > 0) then
    do i = 1, n
  ...
  ! --------- write (skip 2-edge points to avoid artifacts) ---------
