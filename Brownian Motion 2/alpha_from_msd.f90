program alpha_from_msd
  ...
  integer,  parameter :: SMOOTH_WIN = 2
  ...
  close(uin)
  if (nmax < 5) then
    print *, "ERROR: need >=5 data points (got ", nmax, ")"; stop 1
  end if
  ...
  ! central differences in log–log, with eps guards
  do i = 1, n
    if (.not. good(i)) then
      a(i) = 0.0_dp
    else if (i == 1) then
  ...
  ! optional smoothing (moving average over ±win)
  win = SMOOTH_WIN
  if (win > 0) then
    do i = 1, n
  ...
  ! write: skip first/last 2 points (edge artifacts) and any bad rows
