...
program SimulationfullLangevintwopointo
  ...
  print*, "  axes_theta_theory20.txt, axes_theta_sim20.txt"
...
!  - axes_theta_theory20.txt, axes_theta_sim20.txt: two perpendicular lines each
!========================================================
!========================================================
! Theory overlay + diagnostics + principal axes
! - joint_theory20.txt: theory P(x,v) on same grid as hist
! - diagnostics_Tmax20.txt: theory vs sample stats
! - axes_theta_theory20.txt, axes_theta_sim20.txt:
!     two perpendicular lines each (lengths = sqrt(eigvals))
!========================================================
subroutine write_theory_and_diagnostics(tnow, m, gamma, kBT, &
  ...
  real*8 :: PI, tau, a, D, tiny
  real*8 :: sigv2_th, sigx2_th, cov_th, detS, inv11, inv12, inv22
  real*8 :: x_c, v_c, Pth, rho_th, theta_th
  real*8 :: xmean, vmean, xvar, vvar, cov
  real*8 :: rho_sim, theta_sim
  real*8 :: norm_check_the

  ! eigen/axis helpers (theory & sim)
  real*8 :: disc_th, lamP_th, lamM_th, a_len_th, b_len_th
  real*8 :: disc_si, lamP_si, lamM_si, a_len_si, b_len_si
  real*8 :: cth, sth, csi, ssi
  ...
  !======================
  ! 1) exact theory moments (cold start)
  !======================
  ...
  !======================
  ! 2) write theory grid on SAME (x,v) mesh + normalization check
  !======================
  ...
  !======================
  ! 3) sample stats (means, variances, covariance) from particles
  !======================
  ...
  !======================
  ! 5) principal axes with NATURAL lengths (sqrt eigenvalues)
  !======================
  ! --- THEORY eigenvalues/semi-axes ---
  disc_th = dsqrt( (sigx2_th - sigv2_th)**2 + 4.0d0*cov_th**2 )
  lamP_th = 0.5d0*( sigx2_th + sigv2_th + disc_th )   ! major variance
  lamM_th = 0.5d0*( sigx2_th + sigv2_th - disc_th )   ! minor variance
  a_len_th = dsqrt( max(lamP_th, tiny) )   ! major semi-axis (1σ)
  b_len_th = dsqrt( max(lamM_th, tiny) )   ! minor semi-axis (1σ)

  cth = dcos(theta_th);  sth = dsin(theta_th)
  open(newunit=u_ax_th, file="axes_theta_theory20.txt", status='replace', action='write')
  write(u_ax_th,*) "# major & minor axes (theory), lengths = sqrt(eigvals), z=0 for splot"
  ! major axis (along theta_th)
  write(u_ax_th,'(3E20.8)') -a_len_th*cth, -a_len_th*sth, 0.0d0
  write(u_ax_th,'(3E20.8)')  a_len_th*cth,  a_len_th*sth, 0.0d0
  write(u_ax_th,*)
  ! minor axis (perpendicular)
  write(u_ax_th,'(3E20.8)') -b_len_th*sth,  b_len_th*cth, 0.0d0
  write(u_ax_th,'(3E20.8)')  b_len_th*sth, -b_len_th*cth, 0.0d0
  close(u_ax_th)

  ! --- SIM eigenvalues/semi-axes ---
  disc_si = dsqrt( (xvar - vvar)**2 + 4.0d0*cov**2 )
  lamP_si = 0.5d0*( xvar + vvar + disc_si )
  lamM_si = 0.5d0*( xvar + vvar - disc_si )
  a_len_si = dsqrt( max(lamP_si, tiny) )
  b_len_si = dsqrt( max(lamM_si, tiny) )

  csi = dcos(theta_sim); ssi = dsin(theta_sim)
  open(newunit=u_ax_si, file="axes_theta_sim20.txt", status='replace', action='write')
  write(u_ax_si,*) "# major & minor axes (sim), lengths = sqrt(eigvals), z=0 for splot"
  ! major axis
  write(u_ax_si,'(3E20.8)') -a_len_si*csi, -a_len_si*ssi, 0.0d0
  write(u_ax_si,'(3E20.8)')  a_len_si*csi,  a_len_si*ssi, 0.0d0
  write(u_ax_si,*)
  ! minor axis
  write(u_ax_si,'(3E20.8)') -b_len_si*ssi,  b_len_si*csi, 0.0d0
  write(u_ax_si,'(3E20.8)')  b_len_si*ssi, -b_len_si*csi, 0.0d0
  close(u_ax_si)
